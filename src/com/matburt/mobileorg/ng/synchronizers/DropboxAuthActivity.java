...
public class DropboxAuthActivity extends Activity {
	...
	static class ProxyAndroidAuthSession extends AndroidAuthSession {
		...
		public ProxyAndroidAuthSession(Context context, AppKeyPair arg0,
				AccessType arg1) {
			super(arg0, arg1);
	...
	public static AndroidAuthSession createSession(Context context) {
		...
		AppKeyPair appKeyPair = new AppKeyPair(r.getString(
				R.string.dropbox_consumer_key, "invalid"), r.getString(
				R.string.dropbox_consumer_secret, "invalid"));
		SharedPreferences prefs = PreferenceManager
				.getDefaultSharedPreferences(context);
		String key = prefs.getString("dbPrivKey", null);
		String secret = prefs.getString("dbPrivSecret", null);
		if (key != null && secret != null) {
			return new ProxyAndroidAuthSession(context, appKeyPair,
					AccessType.DROPBOX, new AccessTokenPair(key, secret));
		} else {
			return new ProxyAndroidAuthSession(context, appKeyPair,
					AccessType.DROPBOX);
		}
